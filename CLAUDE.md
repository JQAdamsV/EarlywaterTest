# AI Builder — your private instance of tiknix (`pd`)

You are running inside an **isolated, sandboxed copy** of tiknix that belongs to this
customer (`pd`). Everything you see here is theirs to change. You cannot see or
...
- **Their database**: the SQLite file referenced in `conf/config.pd.ini`
...
- Test changes by loading their site (`https://pd.tiknix.com`).
